...
if(isset($_SESSION['userid']))
{
	...
	if($_SESSION['mainid'])
	{
				$q2="SELECT * FROM `doctor` WHERE `id`=".$_SESSION['mainid'];
				@$q_r2=mysqli_query($con,$q2);
				@$data=mysqli_fetch_assoc($q_r2);
				echo "<center><fieldset><legend><font id='error'>Remove docotr</font></legend><table>";
				...
				echo "<tr><td>Specilization:</td><td>".$data['speci']."</td></tr>";
				...
				echo "<tr><td>Doctor bill</td><td>".$data['docbill']."</tr>";
				...
				if(isset($_POST['con']))
				{
					...
					if(!empty($confirm))
					{
						...
						if($num4==1)
						{
							...
							$qw="DELETE FROM `doctor` WHERE `id`=".$_SESSION['mainid'];
							@mysqli_query($con,$q1);
							@mysqli_query($con,$qw);
...
<br /><a href="../doctor.php"><input type="button" value="Cancel"></a>
<?php
}else
{
