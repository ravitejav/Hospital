...
{
require 'next2.php';
	if(isset($_POST['username']) && isset($_POST['pass']) && isset($_POST['passa']) && isset($_POST['fname']) && isset($_POST['sname']) && isset($_POST['gender']) && isset($_POST['qual']) && isset($_POST['add']) && isset($_POST['email']) && isset($_POST['phone']) && isset($_POST['type']) && isset($_POST['timea']) && isset($_POST['timeb']) && isset($_POST['sess']) && isset($_POST['sal']) && isset($_POST['date']))
	{
		$usern=$_POST['username'];$pass1=$_POST['pass'];$pass2=$_POST['passa'];$first=$_POST['fname'];$second=$_POST['sname'];$gender=$_POST['gender'];$qual=$_POST['qual'];$address=$_POST['add'];$email=$_POST['email'];$phone=$_POST['phone'];$type=$_POST['type'];
		$time1=$_POST['timea'];$time2=$_POST['timeb'];$sess=$_POST['sess'];$sal=$_POST['sal'];$date=$_POST['date'];
		if(!empty($usern) && !empty($pass1) && !empty($pass2) && !empty($first) && !empty($second) && !empty($gender) && !empty($qual) && !empty($address) && !empty($email) && !empty($phone) && !empty($type) && !empty($time1) && !empty($time2) && !empty($sess) && !empty($sal) && !empty($date))
        {
        	...
        	if($num2!=1)
        	{
				if($pass1===$pass2)
				{
					if(($sess=='Day' && $time1<$time2) || ($sess=='Night' && $time2<$time1))
					{
						...
						$q3="INSERT INTO `user` VALUES ('','".$usern."','".md5($pass2)."','".$type."')";
						@$query_run2=mysqli_query($con,$q3);
						$q4="SELECT * FROM `user` WHERE `username`='".$usern."'";
						@$q_r4=mysqli_query($con,$q4);
						@$h=mysqli_fetch_assoc($q_r4);
						$query="INSERT INTO `".$type."` VALUES (".$h['id'].",'".$usern."','".md5($pass1)."','".$first."','".$second."','".$gender."','".$qual."','".$address."','".$email."',".$phone.",'".$type."',".$time1.",".$time2.",'".$sess."',".$sal.",'".$date."')";
						@$query_run=mysqli_query($con,$query);
						header('location:../emply.php');
					}
					}else
					{
...
<table>
	...
	<tr><td>Qualification:</td><td><input type="text" name="qual" maxlength="30" value="<?php if(isset($qual)) { echo $qual;} ?>" /></td></tr>
	...
	<tr><td>Type:</td><td><select name="type"><option>medicine</option><option>billing</option><option>recpeptionist</option><option>test</option><option>service</option></select></td></tr>
