...
if(isset($_POST['patid']) && isset($_POST['amt']) && isset($_POST['bill']))
{
 	...
 	if(!empty($patid) && !empty($amt) && !empty($bill))
	{
		...
		if($pat1==1)
		{
			if($pat['medbill']>0)
			{
				$compbill=$pat['medbill']-$amt;
				if($compbill>=0)
				{
					$q="INSERT INTO `billed` VALUES ('',".$patid.",".$_SESSION['recpid'].",".$amt.",'Medicine bill','".date('D M Y',$time)."','".date('H:i:s',$time)."')";
					...
					fclose($handle);
					$query="UPDATE `patient` SET `medbill`=".$compbill." WHERE `id`=".$patid;
					@mysqli_query($con,$query);
										header('location:backend.php');
				}else
				{
					$_SESSION['error']="His bill is the ".$pat['medbill']."\n".",Please pay the exact amount";
				}
			}else
			{
...
<?php } ?>
<fieldset>
	<legend><font id="error">Medicine billing</font></legend>
	<form method="POST" action="billm.php">
	<table>
		...
		<tr><td>For:</td><td><input type="text" required="required" value="Medicine bill" name="bill" readonly></td><td>Date:</td><td><?php echo date('D M Y',$time); ?></td></tr>
