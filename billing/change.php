...
if(isset($_SESSION['recpid']))
{
	...
	if(isset($_POST['old']) && isset($_POST['new']) && isset($_POST['renew']))
	{
		...
		{
			$query="SELECT * FROM `user` WHERE `id`=".$_SESSION['recpid'];
			...
			if($oldhash===$mydata['password'])
			{
				if($new===$renew)
				{
					$query2="UPDATE `user` SET `password`='".md5($new)."' WHERE `id`=".$_SESSION['recpid'];
					$q="UPDATE `billing` SET `password`='".md5($new)."' WHERE `id`=".$_SESSION['recpid'];
