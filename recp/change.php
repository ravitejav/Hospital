...
if(isset($_SESSION['recid']))
{
	...
	if(isset($_POST['old']) && isset($_POST['new']) && isset($_POST['renew']))
	{
		...
		{
			$query="SELECT * FROM `user` WHERE `id`=".$_SESSION['recid'];
			...
			if($oldhash===$mydata['password'])
			{
				if($new===$renew)
				{
					$query2="UPDATE `user` SET `password`='".md5($new)."' WHERE `id`=".$_SESSION['recid'];
					$q="UPDATE `recpeptionist` SET `password`='".md5($new)."' WHERE `id`=".$_SESSION['recid'];
