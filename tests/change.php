...
if(isset($_SESSION['testid']))
{
	...
	if(isset($_POST['old']) && isset($_POST['new']) && isset($_POST['renew']))
	{
		...
		{
			$query="SELECT * FROM `user` WHERE `id`=".$_SESSION['testid'];
			...
			if($oldhash===$mydata['password'])
			{
				if($new===$renew)
				{
					$query2="UPDATE `user` SET `password`='".md5($new)."' WHERE `id`=".$_SESSION['testid'];
					$q="UPDATE `test` SET `password`='".md5($new)."' WHERE `id`=".$_SESSION['testid'];
